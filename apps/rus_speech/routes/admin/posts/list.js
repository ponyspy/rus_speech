...
module.exports = function(Model) {
	...
	module.index = function(req, res, next) {
		Post.find().sort('-date').limit(10).exec(function(err, p_items) {
			if (err) return next(err);

			Post.count().exec(function(err, count) {
				if (err) return next(err);

				res.render('admin/posts', {p_items: p_items, count: Math.ceil(count / 10)});
			});
		});
	};
	...
	module.get_list = function(req, res, next) {
		...
		Query.count(function(err, count) {
			...
			Query.find().sort('-date').skip(+post.context.skip).limit(+post.context.limit).exec(function(err, p_items) {
				if (err) return next(err);
